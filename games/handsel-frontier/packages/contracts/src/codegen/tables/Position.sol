...
library Position {
  // Hex below is the result of `WorldResourceIdLib.encode({ namespace: "frontier", name: "Position", typeId: RESOURCE_TABLE });`
  ResourceId constant _tableId = ResourceId.wrap(0x746266726f6e74696572000000000000506f736974696f6e0000000000000000);
  ...
  function setZ(bytes32 id, int32 z) internal {
    ...
    StoreSwitch.setStaticField(_tableId, _keyTuple, 1, abi.encodePacked((z)), _fieldLayout);
  }
  ...
  function _setZ(bytes32 id, int32 z) internal {
    ...
    StoreCore.setStaticField(_tableId, _keyTuple, 1, abi.encodePacked((z)), _fieldLayout);
  }
   ...
   */
  function set(bytes32 id, int32 x, int32 z) internal {
    bytes memory _staticData = encodeStatic(x, z);
   ...
   * @notice Set the full data using individual values.
   */
  function _set(bytes32 id, int32 x, int32 z) internal {
    bytes memory _staticData = encodeStatic(x, z);
  ...
  function set(bytes32 id, PositionData memory _table) internal {
    bytes memory _staticData = encodeStatic(_table.x, _table.z);
   ...
   */
  function decodeStatic(bytes memory _blob) internal pure returns (int32 x, int32 z) {
    x = (int32(uint32(Bytes.getBytes4(_blob, 0))));

    z = (int32(uint32(Bytes.getBytes4(_blob, 4))));
  }
  ...
  function decode(
    ...
    (_table.x, _table.z) = decodeStatic(_staticData);
  }
   ...
   */
  function encodeStatic(int32 x, int32 z) internal pure returns (bytes memory) {
   ...
   */
  function encode(int32 x, int32 z) internal pure returns (bytes memory, EncodedLengths, bytes memory) {
    bytes memory _staticData = encodeStatic(x, z);
